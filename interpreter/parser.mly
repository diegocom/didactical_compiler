...
%}
	...
	| ideval EQUAL ideval	{ Assign(Den $1, Val(Den $3)) }
	...
	| EINT ideval EQUAL ideval {($2, Newloc(Val(Den($4))))}
	| ESTRING ideval EQUAL ideval {($2, Newloc(Val(Den($4))))}
	| EBOOL ideval EQUAL ideval {($2, Newloc(Val(Den($4))))}
	...
	dicpl:
	| dicps SEMIC dicpl { $1::$3 }
	| dicps  { $1::[] }
	;

	dicp:
	| dicpl	{$1}
	| LPAREN dicp RPAREN {$2}
	| LQPAREN dicpl RQPAREN { $2 }	
	;
	...
	| dicp COMMA LQPAREN RQPAREN COMMA LQPAREN RQPAREN  { ($1,[],[]) }
	| dicp COMMA dicp COMMA LQPAREN RQPAREN  { ($1,$3,[]) }
	| LQPAREN RQPAREN COMMA dicp COMMA LQPAREN RQPAREN  { ([],$4,[]) }
	| dicp COMMA LQPAREN RQPAREN COMMA comml  { ($1,[],$6) }
	| LQPAREN RQPAREN COMMA LQPAREN RQPAREN COMMA comml  { ([],[],$7) }
	| LQPAREN RQPAREN COMMA dicp COMMA comml RPAREN { ([],$4,$6) }
	| LPAREN dicp COMMA dicp COMMA comml RPAREN { ($2,$4,$6) }
	| dicp COMMA dicp COMMA comml { ($1,$3,$5) }
	| comml {([],[],$1)}
	| dicp COMMA comml { ($1,[],$3) }
	| dicp	{ ($1,[],[]) }
