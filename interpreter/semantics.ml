...
and semc (c: com) (r:dval env) (s: mval store) =
  match c with	
	...
	let value1 = (sem (Val(e1)) r s) in
		let value2 = (sem e2 r s) in
	(match v1 with
	   | Dloc(n) -> if(check_const(mvaltoeval(applystore(s1,n)))) then 
			(failwith ("impossibile modificare costante")) 
			else 
			(if(((get_taint value1) == U) && ((get_taint value2) == T)) then  (print_string "\027[41m";failwith("\027[5m Unsafe operation!\027[0m")) else update(s1, n, evaltomval(value2)))
